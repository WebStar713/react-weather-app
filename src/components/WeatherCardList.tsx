...
import { Card, Col, Row, CardHeader } from "shards-react";
...
const WeatherCardList = (props: IProps) => {
  ...
  const renderWeatherCards = (list: IWeatherForecastListItem[] = []) => {
    ...
    if (list && list.length > 0) {
      for (let index = 0; index < list.length; index++) {
        const isLastWeatherForecastOfDay = checkEndOfDay(
          new Date(list[index].dt_txt)
        );
        daily.push(
          <Col key={`Col-${index}`}>
  ...
  return <>{renderWeatherCards(weatherDataList)}</>;
};
